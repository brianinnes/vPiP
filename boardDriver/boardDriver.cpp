...
const std::string CLIENTID("biPlotter");
const std::string TOPIC("#");

const int  QOS = 1;
const long TIMEOUT = 10000L;

// Board connection variables and constants
#define I2C_ADDRESS 0x08
int file_i2c;
int length;
char buffer[60] = {0};
int i2cSlaveAddr = 0x08;
vpipController *controller = 0;
...
public:
	callback(mqtt::async_client& cli, action_listener& listener, vpipController *controller)
...
int main(int argc, char* argv[])
{
	...
	controller = new vpipController(I2C_ADDRESS);
	if (0 < controller->openI2C()) {
	...
	callback cb(client, subListener, controller);
